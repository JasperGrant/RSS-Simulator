...
void translate(float coordX, float coordY, float distance, ros::Publisher vel_pub, geometry_msgs::Twist vel_msg);
...
int main(int argc, char** argv)
{
 ...
 << std::endl << "99 99 to quit" << std::endl;
		...
		while(j<i-1){
			...
			current_angle = angle;
			translate(relativecoords[j][0],relativecoords[j][1],distance, vel_pub, vel_msg);
			j+=1;
		}
...
void translate(float coordX, float coordY, float distance, ros::Publisher vel_pub, geometry_msgs::Twist vel_msg){
	vel_msg.linear.x = coordX/distance;
	vel_msg.linear.y = coordY/distance;
